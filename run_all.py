...
if __name__ == "__main__":
    ...
    # 获取最新的测试报告文件
    report_path = os.path.join(cur_path, "test_report")  # 报告文件夹
    report_file = get_report_file(report_path)  # 获取最新的测试报告
    #邮箱配置
    from base_method import ReadConfig
    emailconfig = ReadConfig.Config()
    sender = emailconfig.get_email("sender")
    psw = emailconfig.get_email("mail_pass")
    smtp_server = emailconfig.get_email("mail_host")
    port = emailconfig.get_email("mail_port")
    receiver = emailconfig.get_email("receiver")
